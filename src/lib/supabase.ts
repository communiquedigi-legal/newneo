...
const getEnvVal = (key: string): string | null => {
  ...
  if (key === 'VITE_SUPABASE_URL') {
    val = typeof import.meta !== 'undefined' ? import.meta.env?.VITE_SUPABASE_URL : undefined;
  } else if (key === 'VITE_SUPABASE_ANON_KEY') {
    val = typeof import.meta !== 'undefined' ? import.meta.env?.VITE_SUPABASE_ANON_KEY : undefined;
  } else if (key === 'SUPABASE_URL') {
    val = typeof import.meta !== 'undefined' ? import.meta.env?.SUPABASE_URL : undefined;
  } else if (key === 'SUPABASE_ANON_KEY') {
    val = typeof import.meta !== 'undefined' ? import.meta.env?.SUPABASE_ANON_KEY : undefined;
  }
...
// Persistent client identifier to prevent echo loops
const localClientId = typeof window !== 'undefined'
  ? ((window as any).__hms_client_id = (window as any).__hms_client_id || 'client-' + Math.random().toString(36).substring(2, 11) + '-' + Date.now())
  : 'server';

// Initialize a shared broadcast channel with self: false
...
export function broadcastDataMutation(table: string, action: 'insert' | 'update' | 'delete' | 'sync') {
  try {
    syncChannel.send({
      type: 'broadcast',
      event: 'data-changed',
      payload: { table, action, senderId: localClientId, timestamp: Date.now() }
    });
    // Trigger a local storage/custom event marked local so components don't double fetch
...
syncChannel
  .on('broadcast', { event: 'data-changed' }, (payload: any) => {
    ...
    if (data && data.senderId === localClientId) {
      return; // Ignore self broadcast
    }
    // Notify React components locally
    window.dispatchEvent(new CustomEvent('supabase-data-sync', { detail: data }));
  })
  .subscribe((status) => {
