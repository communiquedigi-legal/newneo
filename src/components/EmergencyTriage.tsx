...
import { supabaseService, saveEmergencyResuscitationLog } from '@/services/supabaseService';
...
export default function EmergencyTriage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Seed ER Beds asynchronously if missing to prevent side-effects during render
      const existingBeds = storage.get(STORAGE_KEYS_ER_BEDS, []);
  ...
  const handleInterventionSubmit = async () => {
    ...
    if (['Defibrillation', 'Intubation', 'Oxygen', 'Medication', 'Other'].includes(interventionForm.type)) {
      await saveEmergencyResuscitationLog({
        id: `RESUS-${Date.now()}`,
        patientId: selectedCase.patientId || selectedCase.id,
        patientName: selectedCase.name,
        mrn: selectedCase.mrn,
        triageLevel: selectedCase.triageLevel,
  ...
  const handleDispositionSubmit = async () => {
    ...
    // If Admitted to IPD, trigger admission log and ensure patient record exists
    if (dispositionForm.status === 'IPD Transferred') {
      try {
        ...
        let patId = selectedCase.patientId;
        if (!patId) {
          const created = await supabaseService.createPatient({
            name: selectedCase.name,
            phone: selectedCase.phone || 'N/A',
            gender: selectedCase.gender || 'Other',
            age: selectedCase.age || 30,
            address: 'Emergency Admitted',
            status: 'Admitted',
            mrn: selectedCase.mrn || `MRN-EMG-${Date.now().toString().slice(-4)}`
          });
          if (created) patId = created.id;
        }
        ...
        if (!alreadyAdmitted) {
          const newAdmission = {
            ...
            bed_id: null,
            ward: dispositionForm.destination || 'Male General Ward',
            admission_date: new Date().toISOString().split('T')[0],
            admission_time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
            referred_by: 'Emergency Triage',
            case_type: 'Emergency',
            department: 'Emergency',
            urgency: selectedCase.triageLevel === 'Red' ? 'Immediate' : selectedCase.triageLevel === 'Orange' ? 'Urgent' : 'Routine',
            status: 'Admitted',
            clinical_diagnosis: `Admitted via Emergency Triage: ${selectedCase.presentingComplaints || 'Emergency Triage'}`
          };
          
          storage.set('hms_admissions', [newAdmission, ...admissions]);
          try {
            await supabaseService.createAdmission(newAdmission);
          } catch (err) {
            console.log('Supabase admission create fallback to local storage');
          }
          window.dispatchEvent(new CustomEvent('supabase-data-sync'));
          toast.success('Emergency patient admitted to IPD. Bed allocation pending in IPD module.');
        }
      } catch (e) {
