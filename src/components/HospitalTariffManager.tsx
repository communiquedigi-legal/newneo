...
interface HospitalTariffManagerProps {
  ...
  opdCharges?: any;
  setOpdCharges?: React.Dispatch<React.SetStateAction<any>>;
  onExportExcel: () => void;
  onPrintTariff: () => void;
...
export const HospitalTariffManager: React.FC<HospitalTariffManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* SECTION 6.5: OPD BASE CHARGES */}
      {(activeCategory === 'all' || activeCategory === 'policy') && opdCharges && setOpdCharges && (
        <Card className="border border-indigo-200 bg-gradient-to-br from-indigo-50/60 via-white to-indigo-50/30 shadow-sm rounded-2xl overflow-hidden mt-6">
          <CardHeader className="border-b border-indigo-100 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg font-black text-slate-800">
                  OPD & Consultation Base Charges
                </CardTitle>
                <CardDescription className="text-xs text-slate-600">
                  Configure default fee amounts for OPD registration, follow-ups, and doctor consultations. These values will be fetched automatically during patient registration and billing.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-2">
                <Label className="text-xs font-bold text-slate-700">OPD Follow UP Fee (₹)</Label>
                <Input
                  type="number"
                  value={opdCharges.reg || 0}
                  onChange={(e) => {
                    const val = parseFloat(e.target.value) || 0;
                    setOpdCharges(prev => ({ ...prev, reg: val }));
                  }}
                  className="h-10 text-sm font-bold bg-white"
                />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-bold text-slate-700">Appointment Fee (₹)</Label>
                <Input
                  type="number"
                  value={opdCharges.appt || 0}
                  onChange={(e) => {
                    const val = parseFloat(e.target.value) || 0;
                    setOpdCharges(prev => ({ ...prev, appt: val }));
                  }}
                  className="h-10 text-sm font-bold bg-white"
                />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-bold text-slate-700">Default Consultation Fee (₹)</Label>
                <Input
                  type="number"
                  value={opdCharges.consult || 500}
                  onChange={(e) => {
                    const val = parseFloat(e.target.value) || 0;
                    setOpdCharges(prev => ({ ...prev, consult: val }));
                  }}
                  className="h-10 text-sm font-bold bg-white"
                />
              </div>
